...
```bash
mysamapp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mysamapp$ sam local start-api
mysamapp$ curl http://localhost:3000/
```
...
```bash
mysamapp$ sam logs -n HelloWorldFunction --stack-name mysamapp --tail
```
...
```bash
sam delete --stack-name mysamapp
```
